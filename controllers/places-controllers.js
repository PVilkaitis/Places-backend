...
const getPlacesByUserId = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("Could not find places by user id.", 500);
    return next(error);
  }
...
const createPlace = async (req, res, next) => {
  ...
  try {
    //making two changes at the same time, if one of them fails
...
const deletePlace = async (req, res, next) => {
  ...
  let place;
  try {
    // can use populate when there is relation between documents (user.js and place.js ref:)
